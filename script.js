...
Задача: Изпрати снимка пред заведението.`
  },
  {
    title: "Ниво 2",
    type: "photo",
...
където всеки ден се грижиш никой ученик да не остане гладен и намери плика.

Цел: Работното ѝ място.

Задача: Селфи пред стола.`
  },
  {
    title: "Ниво 3",
    type: "free",
...
Показва се приблизителна зона около ул. Любен Каравелов.

Къде ходеше, когато казваше, че играеш карти в Криска?
Там те чака следващото доказателство.

Цел: Къщата на мъжа.

Задача: Намери скрит плик.`
  },
  { title:"Ниво 4", type:"answer", text:"В плика:", question:"Какво е презимето на Борко?", answers:["миленов"] },
...
ВИЖДАМЕ ТЕ!`
  ...
  { title:"Ниво 15", type:"answer", question:"Кой казва – „Каква е тази татуировка? Ти нормална ли си? Мн грозна.“", answers:["митко"] },
  {
    ...
    text:`📍 Отиди на улица Г. Столетов.

Там се намира баба ти.
Пред къщата ще намериш плик.

След като го намериш, продължи към следващото ниво.`
  },
  ...
  { title:"Ниво 21", type:"answer", question:"Коя зодия е Ива?", answers:["водолей"] },
  {
    title:"Ниво 22",
    type:"photo",
    mapLink:"https://www.google.com/maps/search/?api=1&query=работното+място+на+Криска",
    text:`Добре, де, спираме да те разкарваме.

Отиди там, където Криска работи.
Там те чака твоят подарък и плик.
...
Подсказка: буквите образуват мястото, където те чакаме.`,
...
function render(){
  ...
  bar.style.width = `${((current) / levels.length) * 100}%`;
